...
def _compute_signal_scores(
    ...
    """Compute recommendation signals and preserve failures for the caller."""
    try:
        return strategy.compute_signals(
            prices=prices,
            daily_returns=daily_returns,
            as_of=as_of,
        )
    except Exception as exc:
        raise RuntimeError("Failed to compute recommendation signals.") from exc
...
def main() -> None:
    ...
    if not signal_scores:
        print(
            f"No actionable signals were generated by {strategy_name} "
            "for the selected data and date range."
        )
        return
